...
export default {
  install(Vue, options) {
    ...
    Vue.filter('prettyTime', (...args) => {
      args = Array.prototype.slice.call(args);
      const input = args.shift();
      function pretty (m) {
        const now = moment();
        const momentObj = moment(m, moment.ISO_8601);
        if (!momentObj.isValid()) return '';
        if (momentObj.isSame(now, 'day')) return momentObj.format('Сегодня HH:mm');
        return momentObj.format('DD.MM  HH:mm');
      }
      return pretty(input);
    });
  },
